 ...
 *     either USHORT or CONN_SSHORT, it is corrected and set to outputBuf.
 *	
...
int GetCenterofComponentWithBoxNew( void *bufferOut , int *theDim , int *theCent , int *theBox , int nmax , bool bConvert, int nthreshold )
{
	...
	for ( i =0 ; i<dimx ; i++ )
		for (j =0 ; j<dimy ; j++ )
		{
			...
			if (b>0 && bConvert)	
                *( (unsigned char *)bufferOut + j * dimx + i ) = 255;
	...
	{
		if (theNum[i]>0)	{
			*(theCent + 2*i) /= theNum[i] ;
			*(theCent + 2*i+1) /= theNum[i] ;
		}

		if(theNum[i] > nthreshold)
			nRect++;
	}	
